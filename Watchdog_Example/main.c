...
void main(void)
{
    ...
    PinConfig();
    SysTickInit();
    NVIC->ISER[1] = 0x00000008; //Enable PORT1_IRQn
    __enable_irq();
...
{
//    if(timer_flag && !(BUTTON_PORT->IN&BUTTON_PIN))
    if(BUTTON_PORT->IFG & BUTTON_PIN) {
        blink_flag = 1;

        WDT_A->CTL = 0x5A<<8     // Watchdog Password
                    | 1<<5      //Set to ACLK
